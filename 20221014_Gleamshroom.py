...
class GameData:
    def __init__(self):
        self.level_map = None
        self.spores_max = 1 # 現在關卡最大孢子數
        self.spores_left = 1 # 剩餘孢子數
        self.clear_level()
    ...
    def clear_level(self):
        ...
        self.shrooms = []
        self.scroll = [0, 0]
        self.spores = [] # 儲存所有已發射孢子(110行Spore)的訊息
        self.bodies = []
        self.foliage = []
        self.circle_effects = []
        self.glow_shrooms = []
    ...
    def reset_level(self): # 重製關卡
        self.spores_left = self.spores_max # 剩餘孢子數 等於 現在關卡最大孢子數
...
spore_maximums = [5, 3, 15, 7, 45, 15, 45, 99] # 所有關卡最大可用孢子個數
current_level = 1
...
def load_level(gd, level):
    ...
    gd.spores_max = spore_maximums[level - 1] # 使用"現在關卡(level)"從"所有關卡最大可用孢子個數(spore_maximums)"取得"現在關卡最大孢子數(gd.spores_max)"
    gd.reset_level()
...
spore_img = load_img('data/images/spore.png') # 孢子圖片加載
...
while True:
    ...
    for i, spore_obj in sorted(enumerate(gd.spores), reverse=True): # enumerate將"已發射孢子串列(gd.spores)"編號後，使用sorted reverse翻轉
        spore = spore_obj.data # 取的孢子的[位置, 移動方向, 無場景碰撞]
        display.blit(spore_img, (spore[0][0] - gd.scroll[0] - 2, spore[0][1] - gd.scroll[1] - 2))
        # 任務三：孢子移動
        if spore[2]: # 如果孢子還沒與場景發生碰撞
            spore[0][0] += spore[1][0] # 孢子x軸位置更新
            spore[0][1] += spore[1][1] # 孢子y軸位置更新
            if gd.level_map.tile_collide(spore[0]): # 如果與場景地圖發生碰撞
                sounds['thunk'].play() # 播放thunk的音效
                spore[2] = False # 設定碰撞到場景
                for i in range(6): # 火花特效
            ...
            gd.particles.append(Particle(spore[0][0], spore[0][1], 'p', [0, 0], 10, 1.9, custom_color=(255, 255, 255))) # 粒子特效

            for orb in gd.orbs: # 場景中每一個目標球體(紅色圓形)
                if not orb.hit: # 如果紅色目標尚未被擊中
                    orb_r = pygame.Rect(orb.pos[0] - 2, orb.pos[1] - 2, 15, 15)
                    if orb_r.collidepoint(spore[0]): # 如果目標與孢子碰撞
                        sounds['explode'].play() # 播放explode音效
                        sounds['point'].play() # 播放point音效
                        
                        if current_level == 2: # 如果現在關卡(current_level)是第二關
                            gd.tutorial_text = False # 隱藏教學文字

                        gd.spores.pop(i) # 在孢子串列中移除碰撞到的孢子
                        orb.hit = True # 目標球體被擊中
                        gd.circle_effects.append([orb.center, 4, 4, 0.15, 2]) # 圓圈特效
                        gd.circle_effects.append([orb.center, 4, 6, 0.15, 0.5]) # 圓圈特效
                        for i in range(20): # 粒子特效
                        ...
                        if len([orb for orb in gd.orbs if not orb.hit]) == 0:
                            gd.finished_level = 1 # 關卡完成計時
                            gd.actually_finished = True # 設定關卡完成
                            sounds['transition'].play() # 播放音效
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            ...
            if event.key == K_f:
                print(clock.get_fps())
            # current_level: 現在遊戲所在的關卡(1-7)
            # event_key: 鍵盤事件
            # gd.spores_left: 剩餘孢子數
            # 任務二：設定有效建立孢子的條件
            if current_level < 8: # 如果現在關卡在第七關以內(沒有第八關)
                # 偵測到鍵盤方向鍵(發射孢子)
                if event.key in [K_UP, K_DOWN, K_LEFT, K_RIGHT]:
                    if gd.spores_left > 0: # 如果有剩餘的孢子
                        gd.spores_left -= 1 # 孢子子彈減少
                        sounds['shoot'].play() # 播放發射音效
                        if event.key == K_UP: # 如果是按向上鍵
                            gd.player.velocity[1] = 3
                            # gd.spores: 紀錄所有孢子狀態的串列
                            # gd.player.center: 蘑菇中心位置(孢子發射位置)
                            # 任務一: 在孢子串列(gd.spores)中新增(append)一個孢子(Spore)
                            # Hint: Spore([孢子位置, 移動方向, 沒有碰撞場景])
                            # Hint: 移動方向格式[x, y] (x往右越來越大, y往下越來越大)
                            # Hint: 沒有碰撞場景為True/False
                            gd.spores.append(Spore([gd.player.center, [0, -3], True]))
                            gd.player.squish_velocity = -0.15
                            gd.player.scale[1] = 0.8
                            for i in range(6):
                                gd.sparks.append([gd.player.center, -math.pi / 2 + random.random() - 0.5,\
                                                  random.random() * 3 + 2, random.random() * 0.3 + 0.2])
                        if event.key == K_DOWN: # 如果是按向下鍵
                        ...
                        if event.key == K_RIGHT: # 如果是按向右鍵
                            ...
                            for i in range(6):
                                gd.sparks.append([gd.player.center, random.random() - 0.5,\
                                                  random.random() * 3 + 2, random.random() * 0.3 + 0.2])
                        if event.key == K_LEFT: # 如果是按向左鍵
                            gd.player.velocity[0] = 2
                            # 任務一: 在孢子串列中新增一個孢子
                            gd.spores.append(Spore([gd.player.center, [-3, 0], True]))
                            gd.player.squish_velocity = 0.15
                            gd.player.scale[1] = 1.2
                            for i in range(6):
                                gd.sparks.append([gd.player.center, math.pi + random.random() - 0.5,\
                                                  random.random() * 3 + 2, random.random() * 0.3 + 0.2])
                    # gd.tutorial_text: 是否顯示教學文字(True/False)
                    # 回家練習：隱藏提示文字
                    if current_level == 1: # 如果現在關卡(current_level) 是 第一關
                        gd.tutorial_text = False # 隱藏教學文字(gd.tutorial_text)
                if event.key == K_r: # R鍵reset
                    gd.finished_level = 1 # 參數重置
                    sounds['restart'].play() # 播放restart音效
    ...
    # 隨堂測驗：根據關卡/情境變換文字內容
    # gd.tutorial_text：是否要顯示教學文字(True/False)
    # current_level：現在遊戲進行到的關卡(1-7)
    # gd.spores_left：剩餘孢子數
    if gd.tutorial_text: # 如果要顯示教學文字
        if current_level == 1: # 如果是第一關
        ...
        if (current_level == 2) and (gd.spores_left > 0): # 如果是第二關 且 有剩餘孢子數
    ...
    if current_level == 8: # 如果最後一關結束(現在是第八關)
