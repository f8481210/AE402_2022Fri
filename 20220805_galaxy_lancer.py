...
def move_starship(scrn, key): # 移動我機
    ...
    ss_d = 0
    if key[K_UP] == 1: # 如果向上鍵被按下
        ss_y = ss_y - 20 # 飛機往上飛
        if ss_y < 80: # 如果飛機超過上方範圍(y)
            ss_y = 80 # 固定最上面的位置
    if key[K_DOWN] == 1: # 如果向下鍵被按下
        ss_y = ss_y + 20 # 飛機往下飛
        if ss_y > 640: # 如果飛機超過下方範圍(y)
            ss_y = 640 # 固定最下面的位置
    if key[K_LEFT] == 1: # 如果向左鍵被按下
        ss_x = ss_x - 20 # 飛機往左飛
        if ss_x < 40: # 如果飛機超過左邊範圍(x)
            ss_x = 40 # 固定最左邊的位置
    if key[K_RIGHT] == 1: # 如果向右鍵被按下
        ss_x = ss_x + 20 # 飛機往右飛
        if ss_x > 920: # 如果飛機超過右邊範圍(x)
            ss_x = 920 # 固定最右邊的位置
    
    # key_spc = (key_spc+1)*key[K_SPACE] # 按下空白鍵後計算時間
    if key[K_SPACE] == 1: # 如果空白鍵被按下
        key_spc = key_spc + 1 # 開始計算時間
    else: # 如果空白鍵沒有被按下
        key_spc = 0 # 時間歸0
    if key_spc%5 == 1: # 如果間隔5個時間單位
        set_missile(0) # 用set_missile設定子彈
        se_shot.play() # 播放音效
    # key_z = (key_z+1)*key[K_z] # 按下z鍵後開始計算時間
    if key[K_z] == 1: # 如果z鍵被按下
        key_z = key_z + 1 # 開始計算時間
    else: # 如果z鍵沒有被按下
        key_z = 0 # 時間歸0
    if key_z%15 == 1 and ss_shield > 10: # 如果間隔15個時間單位而且生命大於10
        set_missile(1) # 用set_missile設定子彈
        ss_shield = ss_shield - 10 # 生命扣10點
        se_barrage.play() # 播放音效
