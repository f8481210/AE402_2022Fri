...
class GameData:
    ...
    def reset(self):
        ...
        self.enemies = []
        self.game_over = 0 # 不等於0遊戲結束
        self.animations = []
        self.projectiles = []
        self.transition_state = 1
        self.transition_direction = -1
        self.screenshake = 0
        self.completed_level = 0 # 完成關卡初始值=0
...
class Enemy(Entity):
    ...
    def check_player_path(self, gd):
        ...
        if abs(self.center[0] - gd.player.center[0]) < 3:
            return False
        return not bool(gd.level_mask.overlap(pygame.mask.from_surface(surf), (int(top_left[0] - 1 - gd.edges[0]), int(top_left[1] - 1 - gd.edges[2]))))
