...
class Game:
    ...
    def you_win(self):
        self.won = True
        print('You Win!')
        messagebox.showinfo(title = '贏了！',
                            message = str(grid.score) + " 恭喜找到2048！")

    """遊戲失敗"""
    def game_over(self):
        print('Game over!')
        messagebox.showinfo(title = "輸了！",
                             message = str(grid.score) + " QQ")
    ...
    4.在次往左壓縮"""
    def left(self):
        self.grid.left_compress() #壓縮
        self.grid.left_merge() #合併
        self.grid.moved = self.grid.compressed or self.grid.merged
        self.grid.left_compress() #再壓縮
