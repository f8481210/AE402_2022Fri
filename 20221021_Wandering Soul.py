...
I was in a rush and wrote tons of spaghetti. This code probably isn't a very good reference.
"""
...
import data.anim_loader as anim_loader
...
while True:
    ...
    for event in pygame.event.get():
        ...
        # event.type 事件型態
        # event.key 事件按鍵
        if event.type == QUIT: # 如果 事件的型態 是 關閉視窗(QUIT)
            pygame.quit() # 讓pygame停止運作
            sys.exit() # 停止程式碼運作

        if event.type == KEYDOWN: # 如果 事件的型態 是 按下鍵盤(KEYDOWN)
            # 任務二: 移動設定
            # player.pos 玩家位置
            # player_message 要顯示在玩家頭上的訊息
            if event.key == K_e: # 如果 事件按鍵 是 e(K_e)
                print(player.pos) # 列印玩家位置
            if event.key == K_q: # 如果事件按鍵 是 q(K_q)
                player_message = [180, 'hello', 'hello'] # 顯示訊息"hello world"
            # 任務三: 角色移動
            # right/left/down/up 紀錄現在玩家是否要"往右走/左走/下走/上跳"(True/False)
            # tutorial 移動教學(0是還沒完成/1是已經完成)
            if event.key == K_RIGHT: # 如果 事件按鍵 是 向右鍵(K_RIGHT)
                right = True # 向右走 為 是
                if not tutorial: # 如果 還沒完成 移動教學
                    tutorial = 1 # 把移動教學 設成 完成
            if event.key == K_LEFT: # 如果 事件按鍵 是 向左鍵(K_LEFT)
                left = True # 向左走 為 是
            
            if event.key == K_DOWN: # 如果 事件按鍵 是 向下鍵(K_DOWN)
                # 任務四: 狀態轉換
                # right/left/down/up 紀錄現在玩家是否要"往右走/左走/下走/上跳"(True/False)
                # ready_to_exit 通關門是否可以出現(第二關經過特定時間為True/其餘為False)
                # level_name 第幾關(level_1/level_2/...)
                # events['lv1'] 第一關是否"進入靈魂狀態"的學習時間(0:否/其他:是)
                # map_transition 地圖轉換中(關卡轉換)
                if not ready_to_exit: # 如果 不是 第二關的通關門出現後(第二關通關門出現即不可變成靈魂狀態)
                    # 如果 (不是第一關 或 第一關已經進入學習靈魂狀態)  且  不是 地圖轉換中
                    if (level_name!='level_1' or events['lv1']!=0) and not map_transition:
                        if soul_mode == 0: # 如果 不是在靈魂狀態
                            if player_mana > 0: # 如果 玩家有剩餘魔力
                                soul_mode = 1 # 進入靈魂狀態
                                sounds['enter_soul'].play() # 播放 進入靈魂狀態 音效
                                soul.pos = player.pos.copy() # 靈魂位置 與 玩家位置相同(copy)
                                particle_burst(player.center, 50) # 在玩家中心 產生 粒子爆炸特效
                                player_mana -= 1 # 減少 玩家魔力值
                            # 任務五: 提示文字
                            # player_message 要顯示在玩家頭上的訊息
                            else: # 如果玩家沒有剩餘魔力
                                player_message = [200, 'I need more mana!', ''] # 顯示訊息
                else: # 如果還不能離開(前往下一關)
                    player_message = [300, 'I need to move on...', ''] # 顯示訊息
                down = True # 向下走 為 是
            if event.key == K_UP: # 如果 事件按鍵 是 向上鍵(K_UP)
                if not death and (air_timer < 5) and not soul_mode and not map_transition:
                    sounds['jump'].play() # 播放 跳躍 音效
                    player_velocity[1] = -5.2 # 玩家y軸速度向上
                    sparks.append([list(player.rect.bottomleft), math.pi * 0.9, 2 + random.randint(0, 10) / 10, 5, (255, 255, 255)]) # 跳躍的火花特效
                    sparks.append([list(player.rect.bottomright), math.pi * 0.1, 2 + random.randint(0, 10) / 10, 5, (255, 255, 255)]) # 跳躍的火花特效
                up = True # 向上跳 為 是
            
            # 回家作業
            if event.key == K_ESCAPE: # 如果事件按鍵 是 ESCAPE(K_ESCAPE)
                pygame.quit() # 關閉遊戲
                sys.exit() # 關閉遊戲
        
        # 隨堂測驗
        # event.key 事件按鍵(向右鍵K_RIGHT/向左鍵K_LEFT/向下鍵K_DOWN/向上鍵K_UP)
        # right/left/down/up 紀錄現在玩家是否要"往右走/左走/下走/上跳"(True/False)
        if event.type == KEYUP: # 如果 事件的型態 是 放開鍵盤(KEYUP)
            if event.key == K_RIGHT: # 如果 事件按鍵 是 向右鍵
                right = False # 向右走 為 否
            if event.key == K_LEFT: # 如果 事件按鍵 是 向左鍵
                left = False # 向左走 為 否
            if event.key == K_DOWN: # 如果 事件按鍵 是 向下鍵
                down = False # 向下走 為 否
            if event.key == K_UP: # 如果 事件按鍵 是 向上鍵
                up = False # 向上跳 為 否
    ...
    if level_name == 'level_2':
        ...
        if (last < 150) and (events['lv2timer'] >= 150):
            reset = True
            player_message = [420, 'I just need to survive for now.', '']
        if (last < 350) and (events['lv2timer'] >= 350):
            reset = True
        if (last < 500) and (events['lv2timer'] >= 500):
            reset = True
        if (750 < events['lv2timer'] < 750):
            if (events['lv2timer'] % 250 < last % 250) or (events['lv2timer'] % 110 < last % 110):
                ...
                for i in range(11):
                    ...
                    projectiles.append([spawn, vel, 'enemy'])
                sounds['eye_shoot_large'].play()
        if (last < 1000) and (events['lv2timer'] >= 1000):
            reset = True
